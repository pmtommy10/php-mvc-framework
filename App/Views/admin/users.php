                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td class="align-middle"><?= strtoupper($user['sex']) ?></td>
                                <td class="align-middle"><?= $user['email'] ?></td>
                                <td class="align-middle"><?= Util::THdatetime($user['timestamp']) ?></td>
                                <td class="align-middle">
                                    <span class="badge bg-<?= $user['level']['color'] ?>" data-level="<?= $user['level']['value'] ?>"><?= $user['level']['text'] ?></span>
                                </td>
                                <td class="align-middle">
                                    <a href='#' title="คลิกเพื่อปรับสถานะ" class="badge text-decoration-none link-light bg-<?= $user['status']['color'] ?>" data-toggle-status data-user-id="" data-status="<?= $user['status']['value'] ?>"><?= $user['status']['text'] ?></a>
                                </td>
                                <td class="align-middle text-center">
                                    <div class="btn-group">
                                        <button data-toggle="modal" data-user-id="<?= $user['id'] ?>" onclick="fetchUser()" data-target="#Edit" class="btn btn-icon-only btn-warning" title="แก้ไข"><i class="fas fa-edit"></i></button>
...
<div class="modal fade" id="Edit" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <form id="edit-user-form" method="post" enctype="multipart/form-data" class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body overflow-hidden">
                <div class="row my-n3 py-3 bg-light">
                    <div class="col-md-6 form-group">
                        <label>ชื่อสมาชิก <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="edit-id" name="id" required="required" placeholder="Username">
                    </div>
                    <div class="col-md-6">
                        <label>รหัสผ่าน <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <input type="password" id="edit-password" class="form-control" name="password" required="required" placeholder="Password">
                            <div class="input-group-append">
                                <button class="btn-secondary btn" type="button" onclick="$(this).find('i.fas').toggleClass('fa-eye fa-eye-slash').closest('.input-group').find('input').attr('type', function(index, attr) {return attr == 'password' ? 'text' : 'password'})"><i class="fas fa-eye"></i></button>
                                <button class="btn btn-dark" type="button" onclick="$('#edit-password').val(Math.random().toString(36).slice(-8));"><i class="fas fa-sync-alt"></i></button>
                            </div>
                        </div>
                        <small class="text-danger">(ระบุกรณีต้องการเปลี่ยนรหัสผ่าน)</small>
                    </div>
                </div>
                <hr class="mx-n4">
                <div class="row">
...
<script>
    const fetchUser = async () => {
        ...
        const fetched = await fetch(`load/user/${userId}`);
