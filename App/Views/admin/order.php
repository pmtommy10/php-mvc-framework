                    ...
                    <table id="table-<?= $table ?>" class="table table-hover [table-striped] not-datatable text-nowrap w-100 border-bottom">
                        <thead>
                            <tr role="row">
                                ...
                                <th class="align-middle text-center">สถานะชำระเงิน</th>
                                <th class="align-middle text-center">สถานะส่งสินค้า</th>
                                <!-- <th class="align-middle text-center">สลิปชำระเงิน</th> -->
                                <th class="text-center align-middle text-nowrap"><i class="fas fa-search"></i></th>
                                <th class="text-center align-middle text-nowrap">การชำระเงิน</th>
                                <th class="text-center align-middle text-nowrap">การส่งสินค้า</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                        <tfoot class="bg-light">
                            <tr role="row">
                                ...
                                <th data-table="<?= $table ?>" class="align-middle text-center">สถานะชำระเงิน</th>
                                <th data-table="<?= $table ?>" class="align-middle text-center">สถานะส่งสินค้า</th>
                                <!-- <th data-table="<?= $table ?>" class="align-middle text-center">สลิปชำระเงิน</th> -->
                                <th data-table="<?= $table ?>" class="text-center align-middle text-nowrap"><i class="fas fa-search"></i></th>
                                <th data-table="<?= $table ?>" class="text-center align-middle text-nowrap">การชำระเงิน</th>
                                <th data-table="<?= $table ?>" class="text-center align-middle text-nowrap">การส่งสินค้า</th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
...
<div id="shipment-history-dialog" class="modal fade" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-fullscreen">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pl-4 ml-auto">ประวัติการส่งสินค้า</h5>
                <button class="btn-close" data-dismiss="modal"></button>
            </div>
            <div class="modal-body">
...
<div id="shop-payment-dialog" class="modal fade" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 700px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pl-sm-4 ml-auto">จำแนกหัวหน้าสายร้านค้าที่ต้องโอนเงินให้</h5>
                <button class="btn-close" data-dismiss="modal"></button>
            </div>
            <div class="table-full-width">
...
<div id="shop-shipment-dialog" class="modal fade" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 700px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pl-sm-4 ml-auto">จำแนกหัวหน้าสายร้านค้าที่ต้องส่งสินค้าให้</h5>
                <button class="btn-close" data-dismiss="modal"></button>
            </div>
            <div class="table-full-width">
                <table id="shop-shipment-table" class="table table-hover">
                    <thead>
                        <tr>
                            <th style="vertical-align: top !important; line-height: 1;">หัวหน้าสาย</th>
                            <th style="vertical-align: top !important; line-height: 1;">จำนวนสินค้า<br><small class="font-weight-normal text-black-50">(รายการ)</small></th>
...
<script>
    ...
    const shipmentHistoryDialog = document.getElementById('shipment-history-dialog')
    const paymentProductDialog = document.getElementById('shop-payment-dialog')
    const shipmentProductDialog = document.getElementById('shop-shipment-dialog')
    let detailModal;
    let shipmentInfoModal;
    let paymentHistoryModal;
    let shipmentHistoryModal;
    let shopPaymentModal;
    let shopShipmentModal;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        shipmentHistoryModal = new bootstrap.Modal(shipmentHistoryDialog);
        shopPaymentModal = new bootstrap.Modal(paymentProductDialog);
        shopShipmentModal = new bootstrap.Modal(shipmentProductDialog);
        ...
        shipmentTable = $('#shop-shipment-table').DataTable({
            columns: [{
                    name: 'agent',
                    className: 'text-center align-middle'
                },
    ...
    document.body.onclick = async e => {
        ...
        /**
         * Order payment approve / disapprove
         */
        ...
        /** 
         * Order shipment approve / disapprove
         */
        if (target.closest('[shipment-approve]')) {
            const shipmentApproveBtn = target.closest('[shipment-approve]');
            try {
                const orderId = shipmentApproveBtn.dataset.orderId;
                const approve = !!+shipmentApproveBtn.dataset.approve;

                console.log(orderId, approve);
                if (approve) {
                    const shipmentInfo = await fetchJson(`/Admin/load/buyer-agent-address-info/${orderId}`);
                    if (!shipmentInfo) return false;
                    console.log(shipmentInfo);

                    const productId = shipmentInfo.productId;
                    const receiver = shipmentInfo.receiver;
                    const fullname = shipmentInfo.fullname;
                    const address = shipmentInfo.address;

                    const swalSlip = async () => {
                        shopShipmentModal.hide();
                        const container = document.createElement('div');
                        container.innerHTML =
                            `<div class="mb-4">
                                <div>ส่งสินค้าไปที่อยู่ของ</div>
                                ...
                                โปรดอัพโหลดสลิปส่งสินค้าที่ด้านล่างนี้
                            </div>
                            <div class="mb-3">
                                <input type="file" accept="image/*" id="shipment-slip" required="" class="form-control">
                            </div>
                            <p class="text-center">หรือกรอกข้อมูลการส่งสินค้าด้านล่างนี้</p>
                            <div class="mb-3">
                                <select id="shipping-company" class="form-select">
                                    <option value selected>โปรดเลือกผู้ให้บริการ</option>
                                    <option value="th-post">ไปรษณีย์ไทย</option>
                                    ...
                                    <option value="tp">TP Logistics</option>
                            ...
                            <div class="mb-3">
                                <input id="tracking-no" type="text" class="form-control" placeholder="ระบุหมายเลข Tracking Number">
                            </div>`;
                        const confirm = await swal({
                            title: 'อัพโหลดสลิปส่งสินค้า',
                            content: container,
                            icon: 'info',
                            buttons: ['ยกเลิก', 'ยืนยัน']
                        });
                        if (!confirm) return;
                        const slip = container.querySelector('#shipment-slip').files[0];
                        const shippingCompany = container.querySelector('#shipping-company').value;
                        const trackingNo = container.querySelector('#tracking-no').value;
                        if (!slip && !shippingCompany && !trackingNo) return await swal('ไม่พบข้อมูล', 'ไม่พบข้อมูลหลักฐานการจัดส่งสินค้า โปรดเลือกไฟล์สลิปส่งสินค้า หรือระบุข้อมูลการจัดส่งสินค้า', 'error').then(() => swalSlip());
                        if (shippingCompany && !trackingNo) return await swal('ข้อมูลไม่ครบถ้วน', 'โปรดระบุหมายเลขติดตามสินค้า (Tracking Number)', 'warning').then(() => swalSlip());
                        if (!shippingCompany && trackingNo) return await swal('ข้อมูลไม่ครบถ้วน', 'โปรดระบุผู้ให้บริการขนส่ง', 'warning').then(() => swalSlip());
                        return {
                            slip,
                            shippingCompany,
                            trackingNo
                    ...
                    const shippingInfo = await swalSlip();
                    ...
                    if (shippingInfo.slip) {
                        const imgObject = URL.createObjectURL(shippingInfo.slip);
                        const img = document.createElement('img');
                        img.src = imgObject;
                        img.onload = () => URL.revokeObjectURL(img.src);
                        swalOptions.icon = img.src;
                    }
                    if (shippingInfo.shippingCompany && shippingInfo.trackingNo) {
                        const content = document.createElement('div');
                        content.innerHTML = `<hr><p class="mb-1">ข้อมูลการติดตามสินค้า</p><b>${shippingInfo.shippingCompany.toUpperCase()}: ${shippingInfo.trackingNo}</b>`;
                        swalOptions.content = content;
                    }
                    const confirm = await swal({
                        title: 'ตรวจสอบความถูกต้อง',
                        text: 'โปรดตรวจสอบข้อมูลการส่งสินค้าที่คุณระบุ หากถูกต้อง โปรดดำเนินการต่อ',
                    ...
                    if (shippingInfo.slip) formData.append('slip', shippingInfo.slip);
                    formData.append('shipping_company', shippingInfo.shippingCompany);
                    formData.append('tracking_no', shippingInfo.trackingNo);
                    await fadeIn(loader);
                    const result = await fetchJson(`/Admin/action/agent-shipment-approve`, 'POST', formData);
                    if (!result) return;
                    ...
                    const confirm = await swal({
                        title: 'ไม่อนุมัติ',
                        text: `ดำเนินการไม่อนุมัติจัดส่งสินค้าของคำสั่งซื้อ\n#${orderId.padStart(6, 0)}\n\nโปรดกดยืนยันเพื่อดำเนินการต่อ`,
                        icon: 'warning',
                        dangerMode: true,
                        buttons: ['ยกเลิก', 'ยืนยัน'],
                    });
                    ...
                    formData.append('order_id', orderId);
                    formData.append('status', approve);
                    if (shipmentSlip) formData.append('slip', shipmentSlip);
                    await fadeIn(loader);
                    const result = await fetchJson(`/Admin/action/agent-shipment-disapprove`, 'POST', formData);
                    if (!result) return;

                    swal('สำเร็จ', `ดำเนินการแจ้ง${approve ? '' : 'ไม่อนุมัติ'}จัดส่งสินค้าเรียบร้อย`, 'success');
                }
         ...
         * Agent Shipment
         */
        if (target.closest('[agent-shipment]')) {
            const agentShipmentBtn = target.closest('[agent-shipment]');
            try {
                shopShipmentModal.show();

                const orderId = agentShipmentBtn.dataset.orderId;
                const productId = agentShipmentBtn.dataset.productId;
                const receiver = agentShipmentBtn.dataset.userId;
                const fullname = agentShipmentBtn.dataset.fullname;
                const address = agentShipmentBtn.dataset.address;
                const transferAmount = agentShipmentBtn.dataset.transferAmount;

                // const transferInfo = await fetchJson(`/Admin/load/agent-bank-info/${orderId}`);
                // if (!transferInfo) return false;
                // console.log(transferInfo);

                // dtNewData(shipmentTable, transferInfo);
                ...
                const swalSlip = async () => {
                    ...
                    if (!confirm) return shopShipmentModal.show();
                    const slip = container.querySelector('#shipment-slip').files[0];
                    const shippingCompany = container.querySelector('#shipping-company').value;
                    const trackingNo = container.querySelector('#tracking-no').value;
                    if (!slip && !shippingCompany && !trackingNo) return await swal('ไม่พบข้อมูล', 'ไม่พบข้อมูลหลักฐานการจัดส่งสินค้า โปรดเลือกไฟล์สลิปส่งสินค้า หรือระบุข้อมูลการจัดส่งสินค้า', 'error').then(() => swalSlip());
                ...
                swal('สำเร็จ', `ดำเนินการแจ้งจัดส่งสินค้าเรียบร้อย`, 'success').then(() => shopShipmentModal.show());

                const transferInfo = await fetchJson(`/Admin/load/agent-address-info/${orderId}`);
                if (!transferInfo) return false;
                dtNewData(shipmentTable, transferInfo);
        ...
        if (target.closest('[payment-history]')) {
            ...
            try {
                ...
                for (const i in result) {
                    ...
                    col.className = 'col-6 col-md-4 col-lg-3 col-xxl-2 p-2';
                    detail.innerHTML =
                        `<b>ผู้ดำเนินการ:</b> ${result[i].user_id}<br>
                        <b>ผู้รับโอน:</b> ${result[i].receiver}<br>
                        <b>วันที่ดำเนินการ:</b><br>${result[i].timestamp}`
         ...
         * Shipment history
         */
        if (target.closest('[shipment-history]')) {
            const shipmentHistory = target.closest('[shipment-history]');
            const orderId = shipmentHistory.dataset.orderId;

            const shipmentHistoryBody = shipmentHistoryDialog.querySelector('.modal-body');
            try {
                shipmentHistoryBody.innerHTML = '';
                const result = await fetchJson(`/Admin/load/shipment-history/${orderId}`);
                if (!result) return;
                ...
                for (const i in result) {
                    ...
                    col.className = 'col-6 col-md-4 col-lg-3 col-xxl-2 p-2';
                    detail.innerHTML =
                        `<b>SKU:</b> ${result[i].product_id}<br>
                        <b>ผู้ดำเนินการ:</b> ${result[i].user_id}<br>
                        <b>ผู้รับสินค้า:</b> ${result[i].receiver}<br>
                        <b>วันที่ดำเนินการ:</b><br>${result[i].timestamp}<br>
                        ${result[i].tracking_no ? `<b>Tracking No.:</b><br>${result[i].tracking_no}` : ``}`
                    if (result[i].slip) {
                        img.className = 'img-fluid d-block mx-auto mt-3';
                        img.src = `/uploads/shipment/${result[i].slip}`;
                        img.dataset.lity = ''
                        img.dataset.lityTarget = img.src;
                        img.style.cursor = 'pointer';
                        img.title = 'คลิกเพื่อดูเต็มจอ';
                ...
                shipmentHistoryBody.appendChild(list);
                shipmentHistoryModal.show();
            ...
            connectedCallback() {
                ...
                if (shipmentStatus['code'] > 0) {
                    ...
                    shipmentInfoBtn.onclick = async e => {
                        e.preventDefault();
                        detailDialog.addEventListener('hidden.bs.modal', e => shipmentInfoModal.show(), {
                            once: true
                        });
                        detailModal.hide();
